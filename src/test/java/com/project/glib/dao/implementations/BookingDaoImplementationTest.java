...
public class BookingDaoImplementationTest {
    ...
    public void alreadyHasThisBookingTrue() throws Exception {
        ...
        assertTrue(bookingDao
                .alreadyHasThisBooking(booking1.getIdDoc(), booking1.getDocType(), booking1.getIdUser()));
        assertTrue(bookingDao
                .alreadyHasThisBooking(booking2.getIdDoc(), booking2.getDocType(), booking2.getIdUser()));
        assertTrue(bookingDao
                .alreadyHasThisBooking(booking3.getIdDoc(), booking3.getDocType(), booking3.getIdUser()));
    ...
    public void hasActiveBookingFalse() throws Exception {
        ...
        assertFalse(bookingDao.hasActiveBooking(booking1.getIdDoc(), booking1.getDocType()));
        assertFalse(bookingDao.hasActiveBooking(booking2.getIdDoc(), booking2.getDocType()));
        assertFalse(bookingDao.hasActiveBooking(booking3.getIdDoc(), booking3.getDocType()));
    }

    @Test
    public void hasActiveBookingTrue() throws Exception {
        setBookingsActiveAndAdd();
        assertTrue(bookingDao.hasActiveBooking(booking1.getIdDoc(), booking1.getDocType()));
        assertTrue(bookingDao.hasActiveBooking(booking2.getIdDoc(), booking2.getDocType()));
        assertTrue(bookingDao.hasActiveBooking(booking3.getIdDoc(), booking3.getDocType()));
    }

    @Test
    public void hasNotActiveBookingFalse() throws Exception {
        setBookingsActiveAndAdd();
        assertFalse(bookingDao.hasNotActiveBooking(booking1.getIdDoc(), booking1.getDocType()));
        assertFalse(bookingDao.hasNotActiveBooking(booking2.getIdDoc(), booking2.getDocType()));
        assertFalse(bookingDao.hasNotActiveBooking(booking3.getIdDoc(), booking3.getDocType()));
    }

    @Test
    public void hasNotActiveBookingTrue() throws Exception {
        setBookingsNotActiveAndAdd();
        assertTrue(bookingDao.hasNotActiveBooking(booking1.getIdDoc(), booking1.getDocType()));
        assertTrue(bookingDao.hasNotActiveBooking(booking2.getIdDoc(), booking2.getDocType()));
        assertTrue(bookingDao.hasNotActiveBooking(booking3.getIdDoc(), booking3.getDocType()));
    }
    ...
    private void getBookingWithMaxPriority(Booking booking) throws Exception {
        ...
        for (int i = 0; i < cycles; i++) {
            User user1 = new User("login" + i, "password", "passconf",
                    "name", "surname", "address", "79134562845672", User.STUDENT, true);
            userDao.add(user1);
            booking.setIdUser(user1.getId());
            booking.setPriority(booking.getPriority() + additionalPriority);
            bookingDao.add(booking);
        }

        Booking bookingWithMaxPriority = bookingDao.getBookingWithMaxPriority(booking.getIdDoc(), booking.getDocType());
